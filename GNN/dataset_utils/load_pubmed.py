...
def get_pubmed_casestudy(corrected=False, SEED=0, root="./"):
    _, data_X, data_Y, data_pubid, data_edges = parse_pubmed(root)
    ...
    dataset = Planetoid(os.path.join(root, 'datasets/pubmed'), data_name, transform=T.NormalizeFeatures())
    data = dataset[0]
...
def parse_pubmed(root="./"):
    path = os.path.join(root, 'datasets/pubmed/pubmed_orig/data/')
...
def get_raw_text_pubmed(use_text=False, seed=0, root="./"):
    data, data_pubid = get_pubmed_casestudy(SEED=seed, root=root)
    if not use_text:
        return data, None

    f = open(os.path.join(root, 'datasets/pubmed/pubmed_orig/pubmed.json'))
    pubmed = json.load(f)
