...
class PyG_GATEncoder(nn.Module):
    def __init__(self, in_feats, out_feats, hidden_channels=[256, 512], k=2, dropout=0.0):
        super(PyG_GATEncoder, self).__init__()

        self.convs = torch.nn.ModuleList()
        self.bns = torch.nn.ModuleList()

        self.convs.append(GATConv(in_feats, hidden_channels[0]))
        self.bns.append(torch.nn.BatchNorm1d(hidden_channels[0]))

        for i in range(k - 1):
            self.convs.append(GATConv(hidden_channels[i], hidden_channels[i+1]))
